...
$adharincorrect = false;
if (isset($_POST['submit'])) 
    {
        $adhar = $_POST['Aadhar'];
        $Password = $_POST['password'];

        $adhar_search = "select * from registration where adhar = '$adhar' ";
        $query = mysqli_query($con, $adhar_search);

        $adhar_count = mysqli_num_rows($query);
        if ($adhar_count) {

            $adhar_pass = mysqli_fetch_assoc($query);
            $db_pass = $adhar_pass['pass'];

            $_SESSION['username'] = $adhar_pass['name'];
            $_SESSION['id'] = $adhar_pass['sno'];
            $_SESSION['adhar'] = $adhar_pass['adhar'];
...
if ($adharincorrect) {
  // code...
  echo "<div class='alert alert-danger alert-dismissible fade show' role='alert'>
  <strong>Plese!</strong> enter correct Adhar number
  <button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
</div>";
}
?>
                                    ...
                                    <div class="form-outline mb-4">
                                        <input type="number"  name="Aadhar" class="form-control form-control-lg" />
                                    ...
                                    <p class="text-center text-muted mt-5 mb-0">Not Have  an account? <a
                                            href="regi.php" class="fw-bold text-body"><u>Sign here</u></a></p>
